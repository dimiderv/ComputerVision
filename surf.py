...
dimg = cv.drawKeypoints(img1, kp1, None, flags=cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
cv.imshow('main', dimg)
cv.waitKey(0)
...
# Enwsh twn 2 eikonwn
img3 = cv.warpPerspective(img2, M, (img1.shape[1]+600, img1.shape[0]+600))
...
o = cv.drawMatches(img1, desc1[0], img2, desc2[0], matches, None)#o descriptors exei mesa pinaka einai taple
...
# Βρίσκει πώς πρέπει να μετατραπει η πρώτη για να "ταιριάξει" με τη δευτερη
# enwsh 3 kai 4
img6 = cv.warpPerspective(img5, M, (img4.shape[1]+600, img4.shape[0]+600))
