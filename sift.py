...
dimg = cv.drawKeypoints(img1, kp1, None, 5000, flags=cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
cv.imshow('main', dimg)
cv.waitKey(0)
...
# Βρίσκει πώς πρέπει να μετατραπει η πρώτη για να "ταιριάξει" με τη δευτερη
# enwsh 3 kai 4
img6 = cv.warpPerspective(img5, M, (img4.shape[1]+200, img4.shape[0]+400))
